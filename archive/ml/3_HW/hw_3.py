...
def dataclean(train, test):
    ...
    train = train[train['weather'] != 2.5]
    train = train[train['weather'] != 1.5]
    ...
    train['logcount'] = np.log(train['count'])
    train['logcount'] = train.apply(lambda x: 0 if x['logcount'] == np.NINF else x['logcount'], axis=1)
...
def model(train, test):
    ...
    feats = ['daylabel', 'workingday', 'atemp', 'humidity', 'weather', 'windspeed', 'hour', 'holiday']
    # feats = feats + ["h" + str(i) for i in range(0,24)]
    # for cat in ['h1', 'h5', 'h2', 'h3', 'h4', 'h0']:
    #     feats.remove(cat)
    
    for x in range(136, 500, 1):
        print(x)
        x_train, x_test, _, _ = train_test_split(train, train['count'], test_size=0.25, random_state=x)
        # x_train, x_test, _, _ = train_test_split(train, train['count'], test_size=0.25)
        work = train[feats]
        
        # pdb.set_trace()
        # get cross val score
        # for est in range(2, 10, 1):
        #     rfc = GradientBoostingRegressor(n_estimators=130, max_depth=7, random_state=1,
        #                                     max_leaf_nodes=65, min_samples_split=est, max_features=8)
        #     scores = cross_val_score(rfc, work, train['logcount'], cv=5, scoring='neg_mean_squared_error')
        #     print("{}:  {}".format(est, str(scores.mean())))
    
        # pdb.set_trace()
        work = x_train[feats]
        # rfc = GradientBoostingRegressor(n_estimators=130, max_depth=7, max_leaf_nodes=36, random_state=1)
        # rfc.fit(work, x_train['logcount'])
        # mse = mean_squared_error(np.exp(x_test['logcount']), np.exp(rfc.predict(x_test[work.columns])))
        rfc = GradientBoostingRegressor(n_estimators=110, max_depth=18, max_leaf_nodes=36, random_state=x)
...
def create_csv(model, test):
    ...
    # y_pred['count'] = np.exp(y_pred['count'])
    y_pred['count'] = y_pred.apply(lambda x: 4 if x['count'] < 0 else x['count'], axis=1)
